...
def reproduce(mother, father):
    ...
    single_point_crossover = random.randint(0, 2)
    childList = []
    for c in mother:
        childList.append(c)

    childList[single_point_crossover] = father[single_point_crossover]
    child = tuple(childList)
...
def mutate(individual):
    ...
    for c in individual:
        listConverted.append(c)


    bit = random.randint(0, 2)

    if listConverted[bit] == 0:
        listConverted[bit] = 1
    elif listConverted[bit] == 1:
        listConverted[bit] = 0

    mutation = tuple(listConverted)
    return mutation
...
def fitness_function(individual):
    ...
    fitness = 0
    factor = 1
    for i, element in enumerate(reversed(individual)):
        fitness = fitness + element*factor
        factor = factor*2
